...
struct TripleCardScreen: View {
    @EnvironmentObject var userData: UserData
    
    let totalCards = 78
    let radius: CGFloat = 250
    let totalDegrees: Double = 300 
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            VStack{
                ...
                HStack{
                    RectangleView(number: (userData.flippedCards.count > 0) ? userData.flippedCards[0] : -1, color: color(for: (userData.flippedCards.count > 0) ? userData.flippedCards[0] : -1)).animation(.smooth(duration: 1).delay(0.5))
                    Spacer()
                    RectangleView(number: (userData.flippedCards.count > 1) ? userData.flippedCards[1] : -1, color: color(for: (userData.flippedCards.count > 1) ? userData.flippedCards[1] : -1)).animation(.smooth(duration: 1).delay(0.5))
                    Spacer()
                    RectangleView(number: (userData.flippedCards.count > 2) ? userData.flippedCards[2] : -1, color: color(for: (userData.flippedCards.count > 2) ? userData.flippedCards[2] : -1)).animation(.smooth(duration: 1).delay(0.5))
                ...
                TypewriterOverlay(isVisible: $showTypewriterOverlay)
                    .onChange(of: userData.flippedCards.count) { newValue in
                        if newValue == 3 { // 3 kart seçildiğinde
    ...
    var wheelView: some View {
        ...
        ZStack {
            ForEach(0..<totalCards, id: \.self) { index in
                CardView(number: cardOrder[index],
                         color: color(for: cardOrder[index]),
                         isSelected: cardOrder[index] == selectedCard,
                         isFlipped: $flippedCards[index],
                         opacity: cardOpacities[index],
                         isClickable: cardOrder[index] == selectedCard && userData.flippedCards.count < 3)
                ...
                .onTapGesture {
                    ...
                    if actualCardNumber == selectedCard && userData.flippedCards.count < 3 { // En fazla 3 kart açılabilir
                        withAnimation(.bouncy(duration: 0.5)) {
...
struct CardView: View {
    ...
    // Flip animasyonu için state'ler
    ...
    private func getTargetXOffset() -> CGFloat {
        ...
        // 🔹 Ekran genişliğine orantılı X kaymaları
        let xOffsetFactor = screenWidth * 0.32 // Kartların sağa/sola kayma oranı
        
        switch userData.flippedCards.count {
           case 1: return -xOffsetFactor  // 1. kart - sola kayacak
        case 2: return 0               // 2. kart - merkezde
        case 3: return xOffsetFactor   // 3. kart - sağa kayacak
        default: return 0
        }
    }
        
    private func getTargetYOffset() -> CGFloat {
        // 🔹 `CardViewSingle` ile aynı orantılı Y kaydırması
...
struct TripleCardScreen_Previews: PreviewProvider {
    static var previews: some View {
        return TripleCardScreen()
